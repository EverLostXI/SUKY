        ...
        for file in audio_files:
            ...
            try:
                if is_flac:
                    ...
                    # 读取基础信息 (带有默认值回退)
                    title = str(audio.get('TIT2', [file[:-4]]))[2:-2] if 'TIT2' in audio else file[:-4]
                    artist = str(audio.get('TPE1', ['Unknown']))[2:-2] if 'TPE1' in audio else 'Unknown'
                    album_artist_tag = str(audio.get('TPE2', [artist]))[2:-2] if 'TPE2' in audio else artist
                    if album_artist == "Unknown Artist":
                        album_artist = album_artist_tag
